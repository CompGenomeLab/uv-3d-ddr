...
def relabel_nodes(G, max_nodes, attr_key='x'):
    ...
    for node_id, attr in dict(G.nodes(data=True)).items():
        mapping[node_id] = find_closest(lin_centrality_attrs, attr[attr_key][0])
...
def synthesize_graphs(graphs, cat, n, min_size, max_size_Q, max_size_T, margin):
    ...
    while len(a_) < n:
        ...
        if cat == 0:  # graphs sampled from same region (Positive Pair)
            ...
            cat == 1
        ):  # graphs sampled from same region, edge feats changed within margin (Positive Pair)
            graph_tup = sample_same_region(graphs, min_size, max_size_T, a_uniq, b_uniq)
            if graph_tup == None:
                continue
            else:
        ...
        elif cat == 2:  # graphs sampled from different region (Negative Pair)
            ...
            cat == 3
        ):  # graphs sampled from same region, edge feats changed outside margin (Negative Pair)
            graph_tup = sample_same_region(graphs, min_size, max_size_T, a_uniq, b_uniq)
            if graph_tup == None:
                continue
            else:
        ...
        elif cat == 4:  # edge perturbation (add edges) - same region (Negative Pair)
            graph_tup = sample_same_region(graphs, min_size, max_size_T, a_uniq, b_uniq)
            if graph_tup == None:
                continue
            else:
        ...
        elif cat == 5:  # node perturbation (add nodes) - same region (Negative Pair)
            graph_tup = sample_same_region(graphs, min_size, max_size_T, a_uniq, b_uniq)
            if graph_tup == None:
                continue
            else:
